...
def build_granular_graph_collapsed(df: pd.DataFrame) -> nx.Graph:
    """
    Build a detailed migration graph from the DataFrame.
    Most granular version so for each flow there is an edge with full attributes.
    """
    ...
    for index , row in df.iterrows():
        ...
        if hometown_code != first_flow_code:
            # checck if edge exists
            if G.has_edge(hometown_code, first_flow_code):
                G[hometown_code][first_flow_code]['weight'] += 1
            else:
                G.add_edge(hometown_code, first_flow_code, attributes= edge_attributes_1, weight=1)
            # G.add_edge(hometown_code, first_flow_code, attributes=edge_attributes_1)
        ...
        if first_flow_code != current_code:
            if G.has_edge(first_flow_code, current_code):
                G[first_flow_code][current_code]['weight'] += 1
            else:
                G.add_edge(first_flow_code, current_code, attributes=edge_attributes_2, weight=1)
    return G
